...
from BusTicket.settings import MEDIA_ROOT, MEDIA_URL
import json
from django.contrib import messages
from django.contrib.auth.models import User
from django.http import HttpResponse
from reservationApp.forms import UserRegistration, UpdateProfile, UpdatePasswords, SaveCategory, SaveLocation, SaveBus, \
...
from .templatetags.paymail import ultrasender
    ...
    if request.POST:
        ...
        if user is not None:
            if user.is_active:
                ...
                resp['msg'] = "Неверный пароль или имя пользователя1"
        else:
            resp['msg'] = "Неверный пароль или имя пользователя"
...
def schedule_mgt(request):
    context['page_title'] = "Расписание поездок"
...
def scheduled_trips(request):
    if not request.method == 'POST': #ыы
        context['page_title'] = "Список поездок"
        ...
        schedules = Schedule.objects.filter(
            Q(status=1) & Q(schedule__year=year) & Q(schedule__month=month) & Q(schedule__day=day) & Q(
                Q(depart=depart) & Q(destination=destination))).all()
        context['schedules'] = schedules
        ...
        if form.is_valid():
            ...
            if booking is None:
                ...
                messages.success(request,
                                 f'Бронь успешно создана. Ваш номер билета: <b>{booking.code}</b>',
                                 extra_tags='stay')
            else:
                messages.success(request, f'<b>{booking.code}</b> Поезда абронирована успешно.')
            resp['status'] = 'success'
        else:
            for fields in form:
                for error in fields.errors:
...
def view_booking(request, pk=None):
    if pk is None:
        messages.error(request, "Неизвестный номер билета")
        return redirect('booking-page')
    else:
        context['page_title'] = 'Посмотреть бронь'
...
def pay_booked(request):
    resp = {'status': 'failed', 'msg': ''}
    if not request.method == 'POST':
        resp['msg'] = "Неизвестный номер билета"
    else:
        booking = Booking.objects.get(id=request.POST['id'])
        form = PayBooked(request.POST, instance=booking)
        if form.is_valid():
            form.save()
            messages.success(request, f"<b>{booking.code}</b> был оплачен успешно", extra_tags='stay')
            print(booking.schedule.bus)
            ultrasender(booking.mail, booking.code, booking.schedule.bus.bus_number, booking.schedule.schedule.strftime("%m/%d/%Y, %H:%M:%S"), booking.name, booking.seats, booking.total_payable())
            resp['status'] = 'success'
        else:
            for field in form:
...
def delete_booking(request):
    ...
    if request.method == 'POST':
        try:
            ...
            code = booking.code
            booking.delete()
            messages.success(request, f'[<b>{code}</b>] Запись о брони была успешно удалена')
            resp['status'] = 'success'
        except Exception as err:
            resp['msg'] = 'Ошибка.Бронь не удалена'
            print(err)
